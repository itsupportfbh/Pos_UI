...
const WEIGHINGSCALE_COLUMNS: SharedTableColumn<WeighingScaleRow>[] = [
...
export class WeighingScaleComponent {
  ...
  selectedRow: WeighingScaleRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: WeighingScaleRow[] = [];
  tableRows: WeighingScaleRow[] = [];
  ...
  tableColumns = WEIGHINGSCALE_COLUMNS;
  ...
  editRow(row: WeighingScaleRow): void {
  ...
  deactivateRow(row: WeighingScaleRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: WeighingScaleRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: WeighingScaleRow): void {
  ...
  confirmActivateRow(row: WeighingScaleRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: WeighingScaleRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: WeighingScaleRow): MenuItem[] {
