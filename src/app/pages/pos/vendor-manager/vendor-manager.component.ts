...
const VENDORMANAGER_COLUMNS: SharedTableColumn<VendorManagerRow>[] = [
...
@Component({
  ...
  styleUrl: './vendor-manager.component.css'
})
export class VendorManagerComponent {
  ...
  selectedRow: VendorManagerRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: VendorManagerRow[] = [];
  tableRows: VendorManagerRow[] = [];
  userDetails: any = {};
  vendorManagerEntityNo = Number(sessionStorage.getItem('currentMenuEntityNo') || 0);
  vendorManagerRights = {
    View: true,
    Create: true,
    Edit: true,
  ...
  readonly pageTitle = 'Vendor Manager';
  readonly pageSubtitle = 'Manage vendor manager records here.';
  readonly filterTitle = 'Vendor Manager Filters';
  readonly primaryActionLabel = 'Search Vendor Manager';
  readonly secondaryActionLabel = 'Clear Filters';
  readonly showSecondaryAction = true;
  dialogTitle = 'Create Vendor Manager';
  dialogSubtitle = 'Create a new vendor manager record.';
  dialogPrimaryActionLabel = 'Save';
  readonly tableTitle = 'Vendor Manager';
  readonly tableCaption = 'Vendor Manager';
  tableColumns = VENDORMANAGER_COLUMNS;
  ...
  async ngOnInit(): Promise<void> {
    ...
    await this.loadVendorManagerRights();
    this.loadRows();
  }

  async loadVendorManagerRights(): Promise<void> {
    try {
      const orgId = Number(this.userDetails?.OrganizationId || this.userDetails?.OrgId || 0);
      const roleId = Number(this.userDetails?.RoleId || 0);
      const entityNo = Number(this.vendorManagerEntityNo || 0);
      ...
      this.showAddNewButton = this.vendorManagerRights.Create;
      this.showDownloadButton = this.vendorManagerRights.Download;
      this.showRowActions = this.vendorManagerRights.Edit || this.vendorManagerRights.Delete || this.vendorManagerRights.ActiveInActive || this.vendorManagerRights.Print;
    } catch {
      this.vendorManagerRights = {
        View: true,
        Create: false,
      ...
      this.toast.error('Rights Load Failed', 'Unable to load vendor manager role rights. Please check and try again.');
    }
  }
  loadRows(): void {
    this.allRows = [];
  ...
  editRow(row: VendorManagerRow): void {
  ...
  deactivateRow(row: VendorManagerRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  printRow(row: VendorManagerRow): void {
    this.toast.info('Print', 'Print is not connected yet for ' + row.Name + '.');
  }

  openRowActions(menu: any, event: Event, row: VendorManagerRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: VendorManagerRow): void {
  ...
  confirmActivateRow(row: VendorManagerRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: VendorManagerRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: VendorManagerRow): MenuItem[] {
    ...
    if (this.vendorManagerRights.Print) {
