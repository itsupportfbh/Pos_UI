...
const ANNOUNCEMENT_COLUMNS: SharedTableColumn<AnnouncementRow>[] = [
...
export class AnnouncementComponent {
  ...
  selectedRow: AnnouncementRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: AnnouncementRow[] = [];
  tableRows: AnnouncementRow[] = [];
  ...
  tableColumns = ANNOUNCEMENT_COLUMNS;
  announcementRights = { View: true, Create: true, Edit: true, Delete: true, ActiveInActive: true, Print: true, Download: true };
  ...
  userDetails: any = {};
  announcementEntityNo = Number(sessionStorage.getItem("currentMenuEntityNo") || 0);

  async ngOnInit(): Promise<void> {
    this.userDetails = JSON.parse(localStorage.getItem('userDetails') ?? '{}');
    await this.loadAnnouncementRights();
    this.loadRows();
  }

  async loadAnnouncementRights(): Promise<void> {
    const orgId = Number(this.userDetails?.OrganizationId || this.userDetails?.OrgId || 0);
    const roleId = Number(this.userDetails?.RoleId || 0);
    const entityNo = Number(this.announcementEntityNo || 0);
    ...
    try {
      ...
      this.showAddNewButton = this.announcementRights.Create;
      this.showDownloadButton = this.announcementRights.Download;
      this.showRowActions = this.announcementRights.Edit || this.announcementRights.Delete || this.announcementRights.ActiveInActive || this.announcementRights.Print;
    } catch {
      this.announcementRights = { View: true, Create: false, Edit: false, Delete: false, ActiveInActive: false, Print: false, Download: false };
      this.showAddNewButton = false;
      this.showDownloadButton = false;
      this.showRowActions = false;
      this.toast.error('Rights Load Failed', 'Unable to load announcement rights for this role.');
    }
  }
  loadRows(): void {
    this.allRows = [];
  ...
  async exportAnnouncementsAsExcel(): Promise<void> {
    this.downloadLoading = true;
    this.downloadLoadingLabel = 'Excel exporting...';

    try {
      const fileName = 'OrgName-Announcement';
      ...
      if (!exportRows.length) {
        this.toast.warn('No Records', 'No announcements are available to export.');
        return;
      }

      await this.tableExportService.exportExcel(fileName, this.tableColumns, exportRows, 'Announcement');
      this.toast.success('Export Ready', 'Announcement Excel export downloaded successfully.');
    } catch {
      this.toast.error('Export Failed', 'Unable to export announcements to Excel.');
    } finally {
  ...
  async exportAnnouncementsAsPdf(): Promise<void> {
    this.downloadLoading = true;
    ...
    try {
      ...
      await this.tableExportService.exportPdf(fileName, 'Announcement', this.tableColumns, exportRows);
      this.toast.success('Export Ready', 'Announcement PDF export downloaded successfully.');
    } catch {
      this.toast.error('Export Failed', 'Unable to export announcements to PDF.');
    } finally {
      this.downloadLoading = false;
      this.downloadLoadingLabel = 'Exporting...';
    }
  ...
  editRow(row: AnnouncementRow): void {
  ...
  deactivateRow(row: AnnouncementRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: AnnouncementRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: AnnouncementRow): void {
  ...
  confirmActivateRow(row: AnnouncementRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: AnnouncementRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: AnnouncementRow): MenuItem[] {
    ...
    if (this.announcementRights.Print) {
  ...
  printRow(row: AnnouncementRow): void {
    const name = String(row.Name ?? row.Code ?? 'this announcement');
