...
const DATAINITIALIZATION_COLUMNS: SharedTableColumn<DataInitializationRow>[] = [
...
export class DataInitializationComponent {
  ...
  selectedRow: DataInitializationRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: DataInitializationRow[] = [];
  tableRows: DataInitializationRow[] = [];
  ...
  tableColumns = DATAINITIALIZATION_COLUMNS;
  dataInitializationRights = { View: true, Create: true, Edit: true, Delete: true, ActiveInActive: true, Print: true, Download: true };
  ...
  userDetails: any = {};
  dataInitializationEntityNo = Number(sessionStorage.getItem("currentMenuEntityNo") || 0);

  async ngOnInit(): Promise<void> {
    this.userDetails = JSON.parse(localStorage.getItem('userDetails') ?? '{}');
    await this.loadDataInitializationRights();
    this.loadRows();
  }

  async loadDataInitializationRights(): Promise<void> {
    const orgId = Number(this.userDetails?.OrganizationId || this.userDetails?.OrgId || 0);
    const roleId = Number(this.userDetails?.RoleId || 0);
    const entityNo = Number(this.dataInitializationEntityNo || 0);
    ...
    try {
      ...
      this.showAddNewButton = this.dataInitializationRights.Create;
      this.showRowActions = this.dataInitializationRights.Edit || this.dataInitializationRights.Delete || this.dataInitializationRights.ActiveInActive || this.dataInitializationRights.Print;
    } catch {
      this.dataInitializationRights = { View: true, Create: false, Edit: false, Delete: false, ActiveInActive: false, Print: false, Download: false };
      this.showAddNewButton = false;
      this.showRowActions = false;
      this.toast.error('Rights Load Failed', 'Unable to load data initialization rights for this role.');
    }
  }
  loadRows(): void {
    this.allRows = [];
  ...
  editRow(row: DataInitializationRow): void {
  ...
  deactivateRow(row: DataInitializationRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: DataInitializationRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: DataInitializationRow): void {
  ...
  confirmActivateRow(row: DataInitializationRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: DataInitializationRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: DataInitializationRow): MenuItem[] {
    ...
    if (this.dataInitializationRights.Print) {
  ...
  printRow(row: DataInitializationRow): void {
    const name = String(row.Name ?? row.Code ?? 'this data initialization record');
