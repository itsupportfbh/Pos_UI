...
const ORDERSCREEN_COLUMNS: SharedTableColumn<OrderScreenRow>[] = [
...
export class OrderScreenComponent {
  ...
  selectedRow: OrderScreenRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: OrderScreenRow[] = [];
  tableRows: OrderScreenRow[] = [];
  ...
  readonly pageTitle = 'Order Screen';
  readonly pageSubtitle = 'Manage order screen records here.';
  readonly filterTitle = 'Order Screen Filters';
  readonly primaryActionLabel = 'Search Order Screen';
  readonly secondaryActionLabel = 'Clear Filters';
  readonly showSecondaryAction = true;
  dialogTitle = 'Create Order Screen';
  dialogSubtitle = 'Create a new order screen record.';
  dialogPrimaryActionLabel = 'Save';
  readonly tableTitle = 'Order Screen';
  readonly tableCaption = 'Order Screen';
  tableColumns = ORDERSCREEN_COLUMNS;
  ...
  editRow(row: OrderScreenRow): void {
  ...
  deactivateRow(row: OrderScreenRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: OrderScreenRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: OrderScreenRow): void {
  ...
  confirmActivateRow(row: OrderScreenRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: OrderScreenRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: OrderScreenRow): MenuItem[] {
