...
const MEMBERSHIPMANAGEMENT_COLUMNS: SharedTableColumn<MembershipManagementRow>[] = [
...
export class MembershipManagementComponent {
  ...
  selectedRow: MembershipManagementRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: MembershipManagementRow[] = [];
  tableRows: MembershipManagementRow[] = [];
  ...
  readonly pageTitle = 'Membership Management';
  readonly pageSubtitle = 'Manage membership management records here.';
  readonly filterTitle = 'Membership Management Filters';
  readonly primaryActionLabel = 'Search Membership Management';
  readonly secondaryActionLabel = 'Clear Filters';
  readonly showSecondaryAction = true;
  dialogTitle = 'Create Membership Management';
  dialogSubtitle = 'Create a new membership management record.';
  dialogPrimaryActionLabel = 'Save';
  readonly tableTitle = 'Membership Management';
  readonly tableCaption = 'Membership Management';
  tableColumns = MEMBERSHIPMANAGEMENT_COLUMNS;
  ...
  editRow(row: MembershipManagementRow): void {
  ...
  deactivateRow(row: MembershipManagementRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: MembershipManagementRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: MembershipManagementRow): void {
  ...
  confirmActivateRow(row: MembershipManagementRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: MembershipManagementRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: MembershipManagementRow): MenuItem[] {
