...
const REGISTRATION_COLUMNS: SharedTableColumn<RegistrationRow>[] = [
...
export class RegistrationComponent {
  ...
  selectedRow: RegistrationRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: RegistrationRow[] = [];
  tableRows: RegistrationRow[] = [];
  userDetails: any = {};
  registrationEntityNo = Number(sessionStorage.getItem('currentMenuEntityNo') || 0);
  registrationRights = {
    View: true,
    Create: true,
    Edit: true,
  ...
  async ngOnInit(): Promise<void> {
    ...
    await this.loadRegistrationRights();
    this.loadRows();
  }

  async loadRegistrationRights(): Promise<void> {
    try {
      const orgId = Number(this.userDetails?.OrganizationId || this.userDetails?.OrgId || 0);
      const roleId = Number(this.userDetails?.RoleId || 0);
      const entityNo = Number(this.registrationEntityNo || 0);
      ...
      this.showAddNewButton = this.registrationRights.Create;
      this.showDownloadButton = this.registrationRights.Download;
      this.showRowActions = this.registrationRights.Edit || this.registrationRights.Delete || this.registrationRights.ActiveInActive || this.registrationRights.Print;
    } catch {
      this.registrationRights = {
        View: true,
        Create: false,
      ...
      this.toast.error('Rights Load Failed', 'Unable to load gift card registration role rights. Please check and try again.');
    }
  }
  loadRows(): void {
    this.allRows = [];
  ...
  editRow(row: RegistrationRow): void {
  ...
  deactivateRow(row: RegistrationRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  printRow(row: RegistrationRow): void {
    this.toast.info('Print', 'Print is not connected yet for ' + row.Name + '.');
  }

  openRowActions(menu: any, event: Event, row: RegistrationRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: RegistrationRow): void {
  ...
  confirmActivateRow(row: RegistrationRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: RegistrationRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: RegistrationRow): MenuItem[] {
    ...
    if (this.registrationRights.Print) {
