...
const TIPADJUSTMENT_COLUMNS: SharedTableColumn<TipAdjustmentRow>[] = [
...
@Component({
  ...
  templateUrl: './tip-adjustment.component.html',
  styleUrl: './tip-adjustment.component.css'
})
export class TipAdjustmentComponent {
  ...
  selectedRow: TipAdjustmentRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: TipAdjustmentRow[] = [];
  tableRows: TipAdjustmentRow[] = [];
  ...
  tableColumns = TIPADJUSTMENT_COLUMNS;
  ...
  editRow(row: TipAdjustmentRow): void {
  ...
  deactivateRow(row: TipAdjustmentRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: TipAdjustmentRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: TipAdjustmentRow): void {
  ...
  confirmActivateRow(row: TipAdjustmentRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: TipAdjustmentRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: TipAdjustmentRow): MenuItem[] {
