...
const SHIFTCLOSE_COLUMNS: SharedTableColumn<ShiftCloseRow>[] = [
...
export class ShiftCloseComponent {
  ...
  selectedRow: ShiftCloseRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: ShiftCloseRow[] = [];
  tableRows: ShiftCloseRow[] = [];
  ...
  readonly pageTitle = 'Shift Close';
  readonly pageSubtitle = 'Manage shift close records here.';
  readonly filterTitle = 'Shift Close Filters';
  readonly primaryActionLabel = 'Search Shift Close';
  readonly secondaryActionLabel = 'Clear Filters';
  readonly showSecondaryAction = true;
  dialogTitle = 'Create Shift Close';
  dialogSubtitle = 'Create a new shift close record.';
  dialogPrimaryActionLabel = 'Save';
  readonly tableTitle = 'Shift Close';
  readonly tableCaption = 'Shift Close';
  tableColumns = SHIFTCLOSE_COLUMNS;
  ...
  editRow(row: ShiftCloseRow): void {
  ...
  deactivateRow(row: ShiftCloseRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: ShiftCloseRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: ShiftCloseRow): void {
  ...
  confirmActivateRow(row: ShiftCloseRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: ShiftCloseRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: ShiftCloseRow): MenuItem[] {
