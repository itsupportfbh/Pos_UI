...
const DELIVERYOUT_COLUMNS: SharedTableColumn<DeliveryOutRow>[] = [
...
@Component({
  ...
  templateUrl: './delivery-out.component.html',
  styleUrl: './delivery-out.component.css'
})
export class DeliveryOutComponent {
  ...
  selectedRow: DeliveryOutRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: DeliveryOutRow[] = [];
  tableRows: DeliveryOutRow[] = [];
  ...
  readonly pageTitle = 'Delivery Out';
  readonly pageSubtitle = 'Manage delivery out records here.';
  readonly filterTitle = 'Delivery Out Filters';
  readonly primaryActionLabel = 'Search Delivery Out';
  readonly secondaryActionLabel = 'Clear Filters';
  readonly showSecondaryAction = true;
  dialogTitle = 'Create Delivery Out';
  dialogSubtitle = 'Create a new delivery out record.';
  dialogPrimaryActionLabel = 'Save';
  readonly tableTitle = 'Delivery Out';
  readonly tableCaption = 'Delivery Out';
  tableColumns = DELIVERYOUT_COLUMNS;
  deliveryOutRights = { View: true, Create: true, Edit: true, Delete: true, ActiveInActive: true, Print: true, Download: true };
  ...
  userDetails: any = {};
  deliveryOutEntityNo = Number(sessionStorage.getItem("currentMenuEntityNo") || 0);

  async ngOnInit(): Promise<void> {
    this.userDetails = JSON.parse(localStorage.getItem('userDetails') ?? '{}');
    await this.loadDeliveryOutRights();
    this.loadRows();
  }

  async loadDeliveryOutRights(): Promise<void> {
    const orgId = Number(this.userDetails?.OrganizationId || this.userDetails?.OrgId || 0);
    const roleId = Number(this.userDetails?.RoleId || 0);
    const entityNo = Number(this.deliveryOutEntityNo || 0);
    ...
    try {
      ...
      this.showAddNewButton = this.deliveryOutRights.Create;
      this.showRowActions = this.deliveryOutRights.Edit || this.deliveryOutRights.Delete || this.deliveryOutRights.ActiveInActive || this.deliveryOutRights.Print;
    } catch {
      this.deliveryOutRights = { View: true, Create: false, Edit: false, Delete: false, ActiveInActive: false, Print: false, Download: false };
      this.showAddNewButton = false;
      this.showRowActions = false;
      this.toast.error('Rights Load Failed', 'Unable to load delivery out rights for this role.');
    }
  }
  loadRows(): void {
    this.allRows = [];
  ...
  editRow(row: DeliveryOutRow): void {
  ...
  deactivateRow(row: DeliveryOutRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: DeliveryOutRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: DeliveryOutRow): void {
  ...
  confirmActivateRow(row: DeliveryOutRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: DeliveryOutRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: DeliveryOutRow): MenuItem[] {
    ...
    if (this.deliveryOutRights.Print) {
  ...
  printRow(row: DeliveryOutRow): void {
    const name = String(row.Name ?? row.Code ?? 'this delivery out record');
