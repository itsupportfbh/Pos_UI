...
export class CashierInComponent {
  ...
  cashierInRights = { View: true, Create: true, Edit: true, Delete: true, ActiveInActive: true, Print: true, Download: true };
  ...
  userDetails: any = {};
  cashierInEntityNo = Number(sessionStorage.getItem("currentMenuEntityNo") || 0);

  async ngOnInit(): Promise<void> {
    this.userDetails = JSON.parse(localStorage.getItem('userDetails') ?? '{}');
    await this.loadCashierInRights();
    this.loadRows();
  }

  async loadCashierInRights(): Promise<void> {
    const orgId = Number(this.userDetails?.OrganizationId || this.userDetails?.OrgId || 0);
    const roleId = Number(this.userDetails?.RoleId || 0);
    const entityNo = Number(this.cashierInEntityNo || 0);
    ...
    try {
      ...
      this.showAddNewButton = this.cashierInRights.Create;
      this.showRowActions = this.cashierInRights.Edit || this.cashierInRights.Delete || this.cashierInRights.ActiveInActive || this.cashierInRights.Print;
    } catch {
      this.cashierInRights = { View: true, Create: false, Edit: false, Delete: false, ActiveInActive: false, Print: false, Download: false };
      this.showAddNewButton = false;
      this.showRowActions = false;
      this.toast.error('Rights Load Failed', 'Unable to load cashier in rights for this role.');
    }
  }
  ...
  printRow(row: CashierInRow): void {
