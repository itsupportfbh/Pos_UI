...
const MENUSETTING_COLUMNS: SharedTableColumn<MenuSettingRow>[] = [
...
export class MenuSettingComponent {
  ...
  selectedRow: MenuSettingRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: MenuSettingRow[] = [];
  tableRows: MenuSettingRow[] = [];
  ...
  readonly pageTitle = 'Menu Setting';
  readonly pageSubtitle = 'Manage menu setting records here.';
  readonly filterTitle = 'Menu Setting Filters';
  readonly primaryActionLabel = 'Search Menu Setting';
  readonly secondaryActionLabel = 'Clear Filters';
  readonly showSecondaryAction = true;
  dialogTitle = 'Create Menu Setting';
  dialogSubtitle = 'Create a new menu setting record.';
  dialogPrimaryActionLabel = 'Save';
  readonly tableTitle = 'Menu Setting';
  readonly tableCaption = 'Menu Setting';
  tableColumns = MENUSETTING_COLUMNS;
  ...
  editRow(row: MenuSettingRow): void {
  ...
  deactivateRow(row: MenuSettingRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: MenuSettingRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: MenuSettingRow): void {
  ...
  confirmActivateRow(row: MenuSettingRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: MenuSettingRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: MenuSettingRow): MenuItem[] {
