...
const MENUREGISTRATION_COLUMNS: SharedTableColumn<MenuRegistrationRow>[] = [
...
@Component({
  ...
  styleUrl: './menu-registration.component.css'
})
export class MenuRegistrationComponent {
  ...
  selectedRow: MenuRegistrationRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: MenuRegistrationRow[] = [];
  tableRows: MenuRegistrationRow[] = [];
  ...
  readonly pageTitle = 'Menu Registration';
  readonly pageSubtitle = 'Manage menu registration records here.';
  readonly filterTitle = 'Menu Registration Filters';
  readonly primaryActionLabel = 'Search Menu Registration';
  readonly secondaryActionLabel = 'Clear Filters';
  readonly showSecondaryAction = true;
  dialogTitle = 'Create Menu Registration';
  dialogSubtitle = 'Create a new menu registration record.';
  dialogPrimaryActionLabel = 'Save';
  readonly tableTitle = 'Menu Registration';
  readonly tableCaption = 'Menu Registration';
  tableColumns = MENUREGISTRATION_COLUMNS;
  ...
  editRow(row: MenuRegistrationRow): void {
  ...
  deactivateRow(row: MenuRegistrationRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: MenuRegistrationRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: MenuRegistrationRow): void {
  ...
  confirmActivateRow(row: MenuRegistrationRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: MenuRegistrationRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: MenuRegistrationRow): MenuItem[] {
