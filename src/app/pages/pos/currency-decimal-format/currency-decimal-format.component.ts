...
const CURRENCYDECIMALFORMAT_COLUMNS: SharedTableColumn<CurrencyDecimalFormatRow>[] = [
...
@Component({
  selector: 'app-currency-decimal-format',
  standalone: true,
  imports: [
    CommonModule,
    ConfirmDialogModule,
  ...
  templateUrl: './currency-decimal-format.component.html',
...
export class CurrencyDecimalFormatComponent {
  ...
  selectedRow: CurrencyDecimalFormatRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: CurrencyDecimalFormatRow[] = [];
  tableRows: CurrencyDecimalFormatRow[] = [];
  ...
  editRow(row: CurrencyDecimalFormatRow): void {
  ...
  deactivateRow(row: CurrencyDecimalFormatRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: CurrencyDecimalFormatRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: CurrencyDecimalFormatRow): void {
  ...
  confirmActivateRow(row: CurrencyDecimalFormatRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: CurrencyDecimalFormatRow): void {
    this.confirmationService.confirm({
