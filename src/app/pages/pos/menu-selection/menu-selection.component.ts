...
const MENUSELECTION_COLUMNS: SharedTableColumn<MenuSelectionRow>[] = [
...
export class MenuSelectionComponent {
  ...
  selectedRow: MenuSelectionRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: MenuSelectionRow[] = [];
  tableRows: MenuSelectionRow[] = [];
  ...
  readonly pageTitle = 'Menu Selection';
  readonly pageSubtitle = 'Manage menu selection records here.';
  readonly filterTitle = 'Menu Selection Filters';
  readonly primaryActionLabel = 'Search Menu Selection';
  readonly secondaryActionLabel = 'Clear Filters';
  readonly showSecondaryAction = true;
  dialogTitle = 'Create Menu Selection';
  dialogSubtitle = 'Create a new menu selection record.';
  dialogPrimaryActionLabel = 'Save';
  readonly tableTitle = 'Menu Selection';
  readonly tableCaption = 'Menu Selection';
  tableColumns = MENUSELECTION_COLUMNS;
  ...
  editRow(row: MenuSelectionRow): void {
  ...
  deactivateRow(row: MenuSelectionRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: MenuSelectionRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: MenuSelectionRow): void {
  ...
  confirmActivateRow(row: MenuSelectionRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: MenuSelectionRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: MenuSelectionRow): MenuItem[] {
