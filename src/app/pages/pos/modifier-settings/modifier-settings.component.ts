...
const MODIFIERSETTINGS_COLUMNS: SharedTableColumn<ModifierSettingsRow>[] = [
...
export class ModifierSettingsComponent {
  ...
  selectedRow: ModifierSettingsRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: ModifierSettingsRow[] = [];
  tableRows: ModifierSettingsRow[] = [];
  ...
  readonly pageTitle = 'Modifier Settings';
  readonly pageSubtitle = 'Manage modifier settings records here.';
  readonly filterTitle = 'Modifier Settings Filters';
  readonly primaryActionLabel = 'Search Modifier Settings';
  readonly secondaryActionLabel = 'Clear Filters';
  readonly showSecondaryAction = true;
  dialogTitle = 'Create Modifier Settings';
  dialogSubtitle = 'Create a new modifier settings record.';
  dialogPrimaryActionLabel = 'Save';
  readonly tableTitle = 'Modifier Settings';
  readonly tableCaption = 'Modifier Settings';
  tableColumns = MODIFIERSETTINGS_COLUMNS;
  ...
  editRow(row: ModifierSettingsRow): void {
  ...
  deactivateRow(row: ModifierSettingsRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: ModifierSettingsRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: ModifierSettingsRow): void {
  ...
  confirmActivateRow(row: ModifierSettingsRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: ModifierSettingsRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: ModifierSettingsRow): MenuItem[] {
