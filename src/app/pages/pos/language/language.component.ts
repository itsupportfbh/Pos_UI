...
const LANGUAGE_COLUMNS: SharedTableColumn<LanguageRow>[] = [
...
export class LanguageComponent {
  ...
  selectedRow: LanguageRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: LanguageRow[] = [];
  tableRows: LanguageRow[] = [];
  ...
  readonly pageTitle = 'Language';
  readonly pageSubtitle = 'Manage language records here.';
  readonly filterTitle = 'Language Filters';
  readonly primaryActionLabel = 'Search Language';
  readonly secondaryActionLabel = 'Clear Filters';
  readonly showSecondaryAction = true;
  dialogTitle = 'Create Language';
  dialogSubtitle = 'Create a new language record.';
  dialogPrimaryActionLabel = 'Save';
  readonly tableTitle = 'Language';
  readonly tableCaption = 'Language';
  tableColumns = LANGUAGE_COLUMNS;
  languageRights = { View: true, Create: true, Edit: true, Delete: true, ActiveInActive: true, Print: true, Download: true };
  ...
  userDetails: any = {};
  languageEntityNo = Number(sessionStorage.getItem("currentMenuEntityNo") || 0);

  async ngOnInit(): Promise<void> {
    this.userDetails = JSON.parse(localStorage.getItem('userDetails') ?? '{}');
    await this.loadLanguageRights();
    this.loadRows();
  }

  async loadLanguageRights(): Promise<void> {
    const orgId = Number(this.userDetails?.OrganizationId || this.userDetails?.OrgId || 0);
    const roleId = Number(this.userDetails?.RoleId || 0);
    const entityNo = Number(this.languageEntityNo || 0);
    ...
    try {
      ...
      this.showAddNewButton = this.languageRights.Create;
      this.showRowActions = this.languageRights.Edit || this.languageRights.Delete || this.languageRights.ActiveInActive || this.languageRights.Print;
    } catch {
      this.languageRights = { View: true, Create: false, Edit: false, Delete: false, ActiveInActive: false, Print: false, Download: false };
      this.showAddNewButton = false;
      this.showRowActions = false;
      this.toast.error('Rights Load Failed', 'Unable to load language rights for this role.');
    }
  }
  loadRows(): void {
    this.allRows = [];
  ...
  deactivateRow(row: LanguageRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: LanguageRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: LanguageRow): void {
  ...
  confirmActivateRow(row: LanguageRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: LanguageRow): void {
    this.confirmationService.confirm({
  ...
  printRow(row: LanguageRow): void {
    const name = String(row.Name ?? row.Code ?? 'this language');
