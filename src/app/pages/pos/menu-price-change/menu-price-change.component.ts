...
const MENUPRICECHANGE_COLUMNS: SharedTableColumn<MenuPriceChangeRow>[] = [
...
@Component({
  selector: 'app-menu-price-change',
  standalone: true,
  imports: [
    CommonModule,
    ConfirmDialogModule,
  ...
  templateUrl: './menu-price-change.component.html',
  styleUrl: './menu-price-change.component.css'
})
export class MenuPriceChangeComponent {
  ...
  selectedRow: MenuPriceChangeRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: MenuPriceChangeRow[] = [];
  tableRows: MenuPriceChangeRow[] = [];
  ...
  readonly pageTitle = 'Menu Price Change';
  readonly pageSubtitle = 'Manage menu price change records here.';
  readonly filterTitle = 'Menu Price Change Filters';
  readonly primaryActionLabel = 'Search Menu Price Change';
  readonly secondaryActionLabel = 'Clear Filters';
  readonly showSecondaryAction = true;
  dialogTitle = 'Create Menu Price Change';
  dialogSubtitle = 'Create a new menu price change record.';
  dialogPrimaryActionLabel = 'Save';
  readonly tableTitle = 'Menu Price Change';
  readonly tableCaption = 'Menu Price Change';
  tableColumns = MENUPRICECHANGE_COLUMNS;
  ...
  editRow(row: MenuPriceChangeRow): void {
  ...
  deactivateRow(row: MenuPriceChangeRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: MenuPriceChangeRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: MenuPriceChangeRow): void {
  ...
  confirmActivateRow(row: MenuPriceChangeRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: MenuPriceChangeRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: MenuPriceChangeRow): MenuItem[] {
