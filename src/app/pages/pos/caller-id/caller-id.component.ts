...
const CALLERID_COLUMNS: SharedTableColumn<CallerIdRow>[] = [
...
export class CallerIdComponent {
  ...
  selectedRow: CallerIdRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: CallerIdRow[] = [];
  tableRows: CallerIdRow[] = [];
  ...
  deactivateRow(row: CallerIdRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: CallerIdRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: CallerIdRow): void {
  ...
  confirmActivateRow(row: CallerIdRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: CallerIdRow): void {
    this.confirmationService.confirm({
