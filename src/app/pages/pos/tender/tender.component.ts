...
const TENDER_COLUMNS: SharedTableColumn<TenderRow>[] = [
...
export class TenderComponent {
  ...
  deactivateRow(row: TenderRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  openRowActions(menu: any, event: Event, row: TenderRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: TenderRow): void {
  ...
  confirmActivateRow(row: TenderRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: TenderRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: TenderRow): MenuItem[] {
