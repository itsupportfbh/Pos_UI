...
const PAGE_CONFIG: FeaturePageConfig = {
  eyebrow: 'Inventory',
  title: 'Stock Out',
  subtitle: 'Record stock issues and damage.',
  formTitle: `${'Stock Out'} Filters`,
  formDescription: `Static ${'Stock Out'.toLowerCase()} page ready for API integration.`,
  tableTitle: 'Stock Out',
  ...
  summaryCards: [
    { label: 'Records', value: `${[{ code: 'STO-01', name: 'Organic Milk 1L', status: 'Posted' }].length}`, caption: 'Static records shown on this page' },
    { label: 'Module', value: 'Inventory', caption: 'Current functional area' },
    { label: 'Mode', value: 'Static UI', caption: 'Ready for API replacement' }
  ],
  fields: [{ key: 'itemName', label: 'Item', type: 'autocomplete', placeholder: 'Search item', suggestions: ['Organic Milk 1L'] }, { key: 'location', label: 'Location', type: 'select', placeholder: 'Choose location', options: STOCK_LOCATION_OPTIONS }],
  primaryActionLabel: `Search ${'Stock Out'}`,
  secondaryActionLabel: 'Clear Filters',
  tableCaption: 'Stock Out',
  rows: [{ code: 'STO-01', name: 'Organic Milk 1L', status: 'Posted' }],
...
export class StockOutComponent {
  ...
  readonly itemSuggestions = ['Organic Milk 1L'];
  readonly stockLocationOptions = STOCK_LOCATION_OPTIONS;  readonly summaryCards = this.config.summaryCards;
