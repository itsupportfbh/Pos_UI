...
export class StockOutComponent {
  ...
  userDetails: any = {};
  stockOutEntityNo = Number(sessionStorage.getItem('currentMenuEntityNo') || 0);
  stockOutRights = {
    View: true,
    Create: false,
    Edit: false,
  ...
  readonly pageTitle = 'Stock Out';
  readonly pageSubtitle = 'Record stock issues and damage.';
  readonly itemSuggestions: string[] = [];
  readonly stockLocationOptions = STOCK_LOCATION_OPTIONS;  
  readonly filterTitle = `${'Stock Out'} Filters`;
  readonly filterDescription = `API data will be loaded for ${'Stock Out'.toLowerCase()}.`;
  readonly fields: any[] = [{ key: 'itemName', label: 'Item', type: 'autocomplete', placeholder: 'Search item', suggestions: ['Organic Milk 1L'] }, { key: 'location', label: 'Location', type: 'select', placeholder: 'Choose location', options: STOCK_LOCATION_OPTIONS }];
  readonly primaryActionLabel = `Search ${'Stock Out'}`;
  ...
  readonly tableTitle = 'Stock Out';
  readonly tableCaption = 'Stock Out';
  ...
  async loadStockOutRights(): Promise<void> {
    try {
      ...
      this.showDownloadButton = this.stockOutRights.Download;
    } catch {
      this.stockOutRights = {
        View: true,
        Create: false,
      ...
      this.toast.error('Rights Load Failed', 'Unable to load stock out role rights. Please check and try again.');
    }
  }
