...
export class MonthlySalesComponent {
  ...
  userDetails: any = {};
  monthlySalesEntityNo = Number(sessionStorage.getItem('currentMenuEntityNo') || 0);
  monthlySalesRights = {
    View: true,
    Create: false,
    Edit: false,
  ...
  async ngOnInit(): Promise<void> {
    ...
    await this.loadMonthlySalesRights();
  }

  async loadMonthlySalesRights(): Promise<void> {
    try {
      const orgId = Number(this.userDetails?.OrganizationId || this.userDetails?.OrgId || 0);
      const roleId = Number(this.userDetails?.RoleId || 0);
      const entityNo = Number(this.monthlySalesEntityNo || 0);
      ...
      this.showDownloadButton = this.monthlySalesRights.Download;
    } catch {
      this.monthlySalesRights = {
        View: true,
        Create: false,
      ...
      this.toast.error('Rights Load Failed', 'Unable to load monthly sales role rights. Please check and try again.');
    }
  }
