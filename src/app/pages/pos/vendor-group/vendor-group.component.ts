...
const VENDORGROUP_COLUMNS: SharedTableColumn<VendorGroupRow>[] = [
...
@Component({
  ...
  styleUrl: './vendor-group.component.css'
})
export class VendorGroupComponent {
  ...
  selectedRow: VendorGroupRow | null = null;
  rowActionItems: MenuItem[] = [];
  allRows: VendorGroupRow[] = [];
  tableRows: VendorGroupRow[] = [];
  userDetails: any = {};
  vendorGroupEntityNo = Number(sessionStorage.getItem('currentMenuEntityNo') || 0);
  vendorGroupRights = {
    View: true,
    Create: true,
    Edit: true,
  ...
  readonly pageTitle = 'Vendor Group';
  readonly pageSubtitle = 'Manage vendor group records here.';
  readonly filterTitle = 'Vendor Group Filters';
  readonly primaryActionLabel = 'Search Vendor Group';
  readonly secondaryActionLabel = 'Clear Filters';
  readonly showSecondaryAction = true;
  dialogTitle = 'Create Vendor Group';
  dialogSubtitle = 'Create a new vendor group record.';
  dialogPrimaryActionLabel = 'Save';
  readonly tableTitle = 'Vendor Group';
  readonly tableCaption = 'Vendor Group';
  tableColumns = VENDORGROUP_COLUMNS;
  ...
  async ngOnInit(): Promise<void> {
    ...
    await this.loadVendorGroupRights();
    this.loadRows();
  }

  async loadVendorGroupRights(): Promise<void> {
    try {
      const orgId = Number(this.userDetails?.OrganizationId || this.userDetails?.OrgId || 0);
      const roleId = Number(this.userDetails?.RoleId || 0);
      const entityNo = Number(this.vendorGroupEntityNo || 0);
      ...
      this.showAddNewButton = this.vendorGroupRights.Create;
      this.showDownloadButton = this.vendorGroupRights.Download;
      this.showRowActions = this.vendorGroupRights.Edit || this.vendorGroupRights.Delete || this.vendorGroupRights.ActiveInActive || this.vendorGroupRights.Print;
    } catch {
      this.vendorGroupRights = {
        View: true,
        Create: false,
      ...
      this.toast.error('Rights Load Failed', 'Unable to load vendor group role rights. Please check and try again.');
    }
  }
  loadRows(): void {
    this.allRows = [];
  ...
  editRow(row: VendorGroupRow): void {
  ...
  deactivateRow(row: VendorGroupRow): void {
    this.allRows = this.allRows.map((item) => {
      if (item.Id === row.Id) {
        item.IsActive = false;
  ...
  printRow(row: VendorGroupRow): void {
    this.toast.info('Print', 'Print is not connected yet for ' + row.Name + '.');
  }

  openRowActions(menu: any, event: Event, row: VendorGroupRow): void {
    this.selectedRow = row;
    this.rowActionItems = this.getRowActionItems(row);
    menu.toggle(event);
  }

  confirmDeleteRow(row: VendorGroupRow): void {
  ...
  confirmActivateRow(row: VendorGroupRow): void {
    this.confirmationService.confirm({
  ...
  confirmDeactivateRow(row: VendorGroupRow): void {
    this.confirmationService.confirm({
  ...
  private getRowActionItems(row: VendorGroupRow): MenuItem[] {
    ...
    if (this.vendorGroupRights.Print) {
